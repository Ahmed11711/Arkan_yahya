   ...
   public function store(UserPlanWebStoreRequest $request)
{
    ...
    $count_unite = $data['count_unite'] ?? 1;
    $user = $request->get('user');

    $wallet = $this->walletRepository->find($data['wallet_id']);
    if (!$wallet || $wallet->status !== 'active') {
        return $this->errorResponse('This wallet is not available or inactive', 400);
    }
    ...
    $finalPrice = $count_unite * $wallet->amount;

    if ($userBalance->balance < $finalPrice) {
        return $this->errorResponse('Insufficient wallet balance', 400);
    }

    try {
        $userPlan = DB::transaction(function () use ($user, $wallet, $userBalance, $count_unite, $finalPrice) {

            $userBalance->decrement('balance', $finalPrice);

            return $this->userPlanRepository->create([
                'user_id'        => $user['id'],
                'wallet_id'      => $wallet->id,
                'price'          => $finalPrice,
                'status'         => 'active',
